...
class Captcha {
  ...
  drawLine(ctx) {
    ...
    for (let i = 0; i <= lines; i++) {
      ...
      let endX = _.random(0, this.width);
      ctx.strokeStyle = this._randomColor();
      ctx.beginPath();
      ctx.moveTo(0, _.random(0, this.height));
      ctx.quadraticCurveTo(0, _.random(0, this.height), endX, endY)
   ...
   * @private
   */
  _randomColorReadable(min, max) {
    min = min || 1;
	  max = max || 9;
	  const int = this._randomInt(min, max).toString(16);
